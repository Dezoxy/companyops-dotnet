...
| **Content-Security-Policy** on the SPA's responses (script/style/connect-src for the API + Keycloak). | Deferred until the SPA is dockerised behind the edge — a CSP on the API/Keycloak routers now would be the wrong target. | [security.md](security.md) "Transport & headers" |
